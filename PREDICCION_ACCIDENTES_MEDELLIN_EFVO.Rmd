...
```{r}
trcntrl = trainControl(method="cv", number=10)
caret_lm_fit = caret::train(Total_Accidentes~MES+AmoryAmistad+Madre+Padre+Mujer+SEMANA+DIA_SEMANA+Semana.Santa+Feriado, data=Train_D_Dataset,
...
```{r}
y_tr_pred_lm<-predict(caret_lm_fit,Train_D_Dataset[,c("MES","AmoryAmistad","Madre","Padre","Mujer","SEMANA","DIA_SEMANA","Semana.Santa","Feriado")])
...
```{r}
y_test_pred_lm<-predict(caret_lm_fit,Test_D_Dataset[,c("MES","AmoryAmistad","Madre","Padre","Mujer","SEMANA","DIA_SEMANA","Semana.Santa","Feriado")])
