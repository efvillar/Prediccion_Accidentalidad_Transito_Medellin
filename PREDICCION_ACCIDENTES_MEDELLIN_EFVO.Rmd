...
Durante la carga de datos se realizó la verificación de las clases de accidentes e inicialmente se identificaron 6 categorias:
1. Atropello
2. Caida_Ocupante
3. Choque
4. Incendio
...
Sin embargo entre 2014 y 2017 solo se presentaron 17 casos de incendio y en el año 2018, 7 casos.  Dada la poca cantidad de eventos se dejaron finalmente 5 categorias dado que incendio se incorporó a otro:
...
Se transforma la variable FEcha a formato Fecha pues se cargó como factor y se unifica el nombre a FECHA para que coincida con l clave de la tabla Total_Dataset_Freq
...
aggregate(FREQ~ANO*MES, data=Total_Dataset_Freq,FUN=mean)%>%
  plot_ly(x = ~MES,
         ...
         split = ~ANO,
         line=list(width=1))%>%
  layout(title='Promedio diario mensual de ACCIDENTES en Medellín',
         xaxis=list(title="Mes"),
...
aggregate(FREQ~ANO*DIA_NOMBRE, data=Total_Dataset_Freq,FUN=mean)%>%
  plot_ly(x = ~DIA_NOMBRE,
         ...
         split = ~ANO,
         line=list(width=1))%>%
  layout(title='Promedio diario DIA DE LA SEMANA de Accidentes de Tránsito en Medellín',
         xaxis=list(title="Día Semana"),
         yaxis=list(title="No. Accidentes"))
```
...
aggregate(FREQ~ANO*CLASE, data=Total_Dataset_Freq,FUN=mean)%>%
  plot_ly(x = ~CLASE,
         ...
         split = ~ANO,
         line=list(width=1))%>%
  layout(title='Promedio diario CLASE de Accidentes de Tránsito en Medellín',
         xaxis=list(title="Clase"),
         yaxis=list(title="No. Accidentes"))
```


LAs predicciones a realizar serán para cada uno de las 5 clases de accidentes, y para los periodos de tiempo mensual, semanal y diario, por lo que se generarán 15 modelos, 5 clases de accidente por 3 tipos de periodo de predicción.

Se realiza entonces la partición de los datos por frecuencia: M: Mensual, S: Semanal, D: Diario

primero se realiza un reshape del dataframe para consolidar los casos por día y separarlos factores CLASE en columnas
...
## 1. Partición de los datos para entrenamiento y test
