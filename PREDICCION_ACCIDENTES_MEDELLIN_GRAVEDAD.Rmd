...
```{r}
Total_Dataset_Freq$Total_Accidentes <- Total_Dataset_Freq$`SOLO DAÃ‘OS` + Total_Dataset_Freq$MUERTO + Total_Dataset_Freq$HERIDO
```
...
```{r}
Total_Dataset_Freq$Accidentes_Leves <- + Total_Dataset_Freq$`SOLO DAÃ‘OS`
```
...
```{r}
trcntrl = trainControl(method="cv", number=10)
caret_knn_fit_Ch = caret::train(Accidentes_Leves~SEMANA+DIA_SEMANA+Semana.Santa+Feriado+Quincena+Viernes_Desp_Qincena+Viernes_Antes_Puente, data=Train_D_Dataset,
                      method = "knn", trControl = trcntrl,
                      preProcess=c("center", "scale"),
                      tuneLength = 10)
```
...
```{r}
trcntrl = trainControl(method="cv", number=10)
caret_lm_fit = caret::train(HERIDO~Ano_v1+Ano_v1*DIA_SEMANA+SEMANA+DIA_SEMANA+Semana.Santa+Feriado_Lunes+Feriado_Otro+Feria_Flores+Madre, data=Train_D_Dataset,
...
```{r}
mse_tr_lm<-mean((Train_D_Dataset$HERIDO-y_tr_pred_lm)^2) # calcula el mse de entrenamiento
...
mse_test_lm<-mean((Test_D_Dataset$HERIDO-y_test_pred_lm)^2) # calcula el mse de entrenamiento
...
```{r}
glm_fit<-glm(HERIDO~Ano_v1+Ano_v1*DIA_SEMANA+SEMANA+DIA_SEMANA+Semana.Santa+Feriado_Lunes+Feriado_Otro+Feria_Flores+Madre, data=Train_D_Dataset, family = "poisson")
summary(glm_fit)
```
...
mse_tr_glm<-mean((Train_D_Dataset$HERIDO-y_tr_pred_glm)^2) # calcula el mse de entrenamiento
...
mse_test_glm<-mean((Train_D_Dataset$HERIDO-y_test_pred_glm)^2) # calcula el mse de entrenamiento
```
```{r}
...
```{r}
trcntrl = trainControl(method="cv", number=10)
caret_tree_fit = caret::train(HERIDO~Ano_v1+Ano_v1*DIA_SEMANA+SEMANA+DIA_SEMANA+Semana.Santa+Feriado_Lunes+Feriado_Otro+Feria_Flores+Madre,data=Train_D_Dataset,
...
mse_tr_tree<-mean((Train_D_Dataset$HERIDO-y_tr_pred_tree)^2) # calcula el mse de entrenamiento
...
mse_test_tree<-mean((Test_D_Dataset$HERIDO-y_test_pred_tree)^2) # calcula el mse de entrenamiento
...
```{r}
trcntrl = trainControl(method="cv", number=10)
caret_tree_fit_m = caret::train(MUERTO~Ano_v1+Ano_v1*DIA_SEMANA+SEMANA+DIA_SEMANA+Semana.Santa+Feriado_Lunes+Feriado_Otro+Feria_Flores+Madre,data=Train_D_Dataset,
                              method = "rpart", trControl = trcntrl,
                      parms = list(split = "gini"),
                      preProcess=c("center", "scale"),
                      tuneLength = 10)
...
mse_tr_tree_m<-mean((Train_D_Dataset$MUERTO-y_tr_pred_tree_m)^2) # calcula el mse de entrenamiento
...
mse_test_tree_m<-mean((Test_D_Dataset$MUERTO-y_test_pred_tree_m)^2) # calcula el mse de entrenamiento
...
```{r}
trcntrl = trainControl(method="cv", number=10)
caret_tree_fit_sd = caret::train(SOLO_DANOS~Ano_v1+Ano_v1*DIA_SEMANA+SEMANA+DIA_SEMANA+Semana.Santa+Feriado_Lunes+Feriado_Otro+Feria_Flores+Madre,data=Train_D_Dataset,
                              method = "rpart", trControl = trcntrl,
                      parms = list(split = "gini"),
                      preProcess=c("center", "scale"),
                      tuneLength = 10)
...
mse_tr_tree_sd<-mean((Train_D_Dataset$SOLO_DANOS-y_tr_pred_tree_sd)^2) # calcula el mse de entrenamiento
...
mse_test_tree_sd<-mean((Test_D_Dataset$SOLO_DANOS-y_test_pred_tree_sd)^2) # calcula el mse de entrenamiento
...
trcntrl = trainControl(method="cv", number=10)
caret_rf_fit = caret::train(HERIDO~Ano_v1+Ano_v1*DIA_SEMANA+SEMANA+DIA_SEMANA+Semana.Santa+Feriado_Lunes+Feriado_Otro+Feria_Flores+Madre, data=Train_D_Dataset,
...
mse_tr_rf<-mean((Train_D_Dataset$HERIDO-y_tr_pred_rf)^2) # calcula el mse de entrenamiento
...
mse_test_rf<-mean((Test_D_Dataset$HERIDO-y_test_pred_rf)^2) # calcula el mse de entrenamiento
